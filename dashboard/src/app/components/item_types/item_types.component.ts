...
import { EditItem_typesComponent } from './modals/edit.item_types.component';

/*
 * "Datagrid" component for displaying instance of Item_types
...
export class Item_typesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.root_resolve_url = new FormControl('');
    this.root_resolve_url.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['root_resolve_url.like'] = this.root_resolve_url.value + '%';
        this.getData();
      });
  }
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.item_types_Get(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.item_types_count_Get(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditItem_typesComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Item_types item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Item_types item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
