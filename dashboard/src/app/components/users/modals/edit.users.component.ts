...
/*
 * Modal dialog for editing your existing Users entity types, and/or
...
export class EditUsersComponent {
   ...
   */
  private createColumns: string[] = ['username', 'password'];
  private updateColumns: string[] = ['username', 'password'];
  private primaryKeys: string[] = ['username'];
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.users_Put(this.data.entity).subscribe(res => {
      ...
      this.service.users_Post(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
