...
export class TemplatesComponent implements OnInit {
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.templates_Get(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.templates_count_Get(cloned).subscribe(res2 => {
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Templates item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
